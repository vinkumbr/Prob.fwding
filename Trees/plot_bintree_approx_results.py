...
ax1.plot(n,upper_approx,'go', label='upper bound from Prop IV.2',ms=3)
ax1.plot(n,upper_tight,'rd', label='upper bound from (5)',ms=3)
ax1.plot(n,minp_expr,'b-', label='numerically solving (4)',lw=2)
ax1.plot(n,lower_tight,'r--', label='lower bound from (6)',lw=2)
ax1.plot(n,lower_approx,'g+', label='lower bound from Prop IV.1',ms=3)
...
ax2.plot(n,trans_upper_approx,'go', label='upper bound using Prop IV.2',ms=3)
ax2.plot(n,tau_upper_tight,'rd', label='upper bound using (5)',ms=3)
ax2.plot(n,trans_expr,'b-', label='numerically solving (4)',lw=2)
ax2.plot(n,tau_lower_tight,'r--', label='lower bound using (6)',lw=2)
ax2.plot(n,trans_lower_approx,'g+', label='lower bound using Prop IV.1',ms=3)
