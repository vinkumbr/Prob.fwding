...
# Transmissions for filename = RGG7_101_int_4.5.txt
...
# Transmissions for filename = RGG8_101_int_4.5.txt
...
# Transmissions for filename = RGG9_101_int_4.5.txt
...
avg_trans = [avg_trans[i]/instantiations for i in range(len(avg_trans))]
...
#theta_lbda_kndelta = [theta_lambda_251[int((round(l,2)-1)/0.01)] for l in lbda_pkndelta]
print(theta_lbda_kndelta)
...
avg_trans_perpkt = [avg_trans[i]/(k+i) for i in range(len(avg_trans))]
tau_kndelta_ergodic_perpkt = [m**2*lbda_pkndelta[i]*theta_lbda_kndelta_square[i] for i in range(len(lbda_pkndelta))]
tau_kndelta_simu_perpkt = [tau_kndelta_simu[i]/(k+i) for i in range(len(tau_kndelta_simu))]
...
ax1.plot(n,tau_kndelta_simu,label='Trace 0')
#ax1.plot(n,trans1,label='Trace 1')
#ax1.plot(n,trans2,label='Trace 2')
#ax1.plot(n,trans3,label='Trace 3')
#ax1.plot(n,trans4,label='Trace 4')
#ax1.plot(n,trans5,label='Trace 5')
#ax1.plot(n,trans6,label='Trace 6')
#ax1.plot(n,trans7,label='Trace 7')
#ax1.plot(n,trans8,label='Trace 8')
#ax1.plot(n,trans9,label='Trace 9')
ax1.plot(n,avg_trans,'o',label='Average')
ax1.plot(n,tau_kndelta_ergodic,'--',label='Ergodic')
ax1.set_title('Expected total number of transmissions')
ax1.legend()

ax2.plot(n,tau_kndelta_simu_perpkt,label='Trace 0')
ax2.plot(n,avg_trans_perpkt,'o',label='Average')
ax2.plot(n,tau_kndelta_ergodic_perpkt,'--',label='Ergodic')
ax2.set_xlabel('Number of coded packets (n)')
ax2.set_title('Expected total number of transmisssions per packet')
ax2.legend()
plt.show()
