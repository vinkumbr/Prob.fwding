...
#theta_half_lbda_p = f(half_lbda_p)
...
recs_ergodic_upper = [m**2*lbda_p[i]*theta_lbda_p_square[i]+m**2*lbda_q[i]*theta_lbda_p[i] for i in range(len(lbda_p))]
recs_ergodic_lower = [m**2*lbda*theta_lbda_p_square[i] for i in range(len(lbda_p))]
#recs_ergodic_lower = [m**2*lbda*theta_half_lbda_p[i]*theta_lbda_p[i] for i in range(len(lbda_p))]

#another approx using theta^+ idea

thetaplus = theta_lbda_p/p
recs_plus_approx = [m**2*lbda*thetaplus[i]*theta_lbda_p[i] for i in range(len(lbda_p))]


fig,ax = plt.subplots(1,1)
fig.suptitle('For an RGG in a 101 by 101 grid with connection radius =1 and intensity 4.5.')
ax.plot(p,recs_ergodic_upper,"o",label='upper bound on receivers')
ax.plot(p,recs_ergodic_lower,"+",label='lower bound on receivers')
ax.plot(p,recs_from_simu,label='from simulations')
#ax.plot(p,recs_plus_approx,"--",label='from $\\theta^+(\lambda,p) = \\theta(\lambda p)/p$')
ax.set_title('Expected number of receivers of the packet from the origin (averaged over 100 realizations of the RGG and 100 probabilistic forwarding protocol iterations for each graph)')
